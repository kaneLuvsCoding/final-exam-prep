...
export default function StudyHub() {
  ...
  const [semesterMajorMappings, setSemesterMajorMappings] = useState([]);

  const [activeSemesterId, setActiveSemesterId] = useState(() => {
    return localStorage.getItem('selectedSemesterId') || null;
  });
  const [activeMajorId, setActiveMajorId] = useState(() => {
    return localStorage.getItem('selectedMajorId') || null;
  });

  const [isSelectionOpen, setIsSelectionOpen] = useState(false);
  const [hoveredSemesterId, setHoveredSemesterId] = useState(null);
  ...
  useEffect(() => {
    async function loadInitialData() {
      const [semRes, majRes, mapRes] = await Promise.all([
        supabase.from('semesters').select('*').order('sort_order', { ascending: true }),
        supabase.from('majors').select('*').order('id'),
        supabase.from('semester_majors').select('*')
      ]);
      if (semRes.data) {
        setAvailableSemesters(semRes.data);
        if (!activeSemesterId && semRes.data.length > 0) {
          // Find Sem VII if possible, else first
          const semVII = semRes.data.find(s => s.name === "Sem VII");
          setActiveSemesterId(semVII ? semVII.id : semRes.data[0].id);
        }
      }
      if (majRes.data) {
        setAvailableMajors(majRes.data);
        if (!activeMajorId && majRes.data.length > 0) {
          const seMajor = majRes.data.find(m => m.name === "SE");
          setActiveMajorId(seMajor ? seMajor.id : majRes.data[0].id);
        }
      }
      if (mapRes.data) {
        setSemesterMajorMappings(mapRes.data);
      }
    }
    loadInitialData();
  }, []);
  ...
  useEffect(() => {
    if (activeSemesterId) localStorage.setItem('selectedSemesterId', activeSemesterId);
    if (activeMajorId) localStorage.setItem('selectedMajorId', activeMajorId);
  }, [activeSemesterId, activeMajorId]);

  useEffect(() => {
    async function loadData() {
      if (!activeSemesterId || !activeMajorId) return;

      try {
        // 1. Fetch subjects allowed for this major
        const { data: allowedSubjects, error: subError } = await supabase
          .from('major_subjects')
          .select('subjects(id, name)')
          .eq('major_id', activeMajorId);

        if (subError) throw subError;

        const allowedSubjectNames = allowedSubjects?.map(s => s.subjects?.name).filter(Boolean) || [];
        const allowedSubjectIds = allowedSubjects?.map(s => s.subjects?.id).filter(Boolean) || [];

        // 2. Fetch questions for this semester and ANY of the allowed subjects
        let query = supabase
          .from('questions')
        ...
        const grouped = {};
        // Initialize grouped with allowed subjects to ensure they show up in sidebar even if empty
        allowedSubjectNames.forEach(name => {
          grouped[name] = {};
        });
        ...
        data.forEach(q => {
          ...
          // Only include if it's in the allowed subjects (double check)
          if (!allowedSubjectNames.includes(sName)) return;
        ...
        // Ensure activeSubject is valid for the new major
  ...
  return (
    <div className="h-screen bg-slate-50 dark:bg-[#171717] text-slate-900 dark:text-slate-100 flex flex-col overflow-hidden relative">
      ...
      {/* REWARD POP-UP MODAL */}
      <div
        className={`fixed inset-0 z-[100] flex items-center justify-center p-4 bg-[#171717]/60 dark:bg-[#171717]/80 backdrop-blur-sm transition-all duration-300 ${showCompletionModal ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'
          }`}
      >
        <div className={`bg-white dark:bg-slate-800 rounded-3xl p-6 md:p-8 max-w-sm w-full shadow-2xl transform transition-all text-center duration-300 ${showCompletionModal ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'
          }`}>
      ...
      <header className="bg-gradient-to-r from-[#045c66] via-[#077d8a] to-[#0996a6] text-white shadow-lg z-40 shrink-0 border-b border-[#045c66]/30">
        <div className="px-4 md:px-8 py-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          {/* Row 1: Logo, Toggles, and Mobile Hamburger */}
          <div className="flex items-center justify-between shrink-0 w-full md:w-auto">
            <div className="flex items-center gap-3">
              <h1 className="text-xl md:text-3xl font-black tracking-tighter flex items-center gap-2 drop-shadow-sm mr-auto md:mr-2">
                S.T.U.D.Y
              </h1>
            ...
            <button
              className="md:hidden p-2 bg-white/10 hover:bg-white/20 rounded-xl transition-all backdrop-blur-sm shadow-sm"
          ...
          {/* Row 2: Study Path Button */}
          <div className="flex justify-center w-full md:w-auto">
            <div className="relative group/main w-full md:w-auto" ref={selectionRef}>
              <button
                type="button"
                onClick={() => setIsSelectionOpen(!isSelectionOpen)}
                className={`${topActionButtonClass} flex items-center justify-between md:justify-start gap-2 px-4 md:px-4 w-full md:w-auto min-w-[130px] md:min-w-[160px] active:scale-95 transition-all ${isSelectionOpen ? 'bg-white/20 border-white/40 shadow-inner' : ''}`}
                title="Change Semester/Major"
              >
                <div className="flex flex-col items-start leading-none gap-0.5">
                  <span className="text-[10px] font-black uppercase tracking-tighter opacity-70">Study Path</span>
                  <span className="text-[11px] font-bold whitespace-nowrap overflow-hidden text-ellipsis max-w-[150px] md:max-w-[110px]">
                    {availableSemesters.find(s => s.id == activeSemesterId)?.name || 'Sem'} • {availableMajors.find(m => m.id == activeMajorId)?.name || 'Major'}
                  </span>
                </div>
                <svg className={`w-3 h-3 transition-transform duration-300 ${isSelectionOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M19 9l-7 7-7-7" />
                </svg>
              </button>

              {isSelectionOpen && (
                <div className="absolute top-full left-0 mt-3 w-64 md:w-56 bg-white/95 dark:bg-[#1f1f1f]/95 rounded-3xl shadow-[0_20px_50px_-12px_rgba(0,0,0,0.3)] border border-slate-200/50 dark:border-slate-800/50 py-3 z-[60] backdrop-blur-2xl animate-in fade-in zoom-in-95 duration-200 origin-top">
                  <div className="px-5 py-2 mb-1">
                    <span className="text-[10px] font-black uppercase tracking-widest text-[#077d8a] dark:text-[#58b8c1] opacity-60">Choose Semester</span>
                  </div>
                  {availableSemesters.map(s => (
                    <div key={s.id} className="relative">
                      <div
                        onClick={(e) => {
                          e.stopPropagation();
                          setHoveredSemesterId(hoveredSemesterId === s.id ? null : s.id);
                        }}
                        className={`px-5 py-3 text-sm font-bold flex items-center justify-between cursor-pointer transition-all duration-200 ${hoveredSemesterId === s.id ? 'bg-[#077d8a]/10 text-[#077d8a] translate-x-1' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800/40'}`}
                      >
                        <span>{s.name}</span>
                        <svg className={`w-3 h-3 transition-all duration-200 ${hoveredSemesterId === s.id ? 'opacity-100 rotate-90 md:rotate-0' : 'opacity-30'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M9 5l7 7-7 7" />
                        </svg>
                      </div>

                      {hoveredSemesterId === s.id && (
                        <div className="relative md:absolute md:left-full md:top-0 md:-ml-1 w-full md:w-44 bg-slate-50/50 dark:bg-slate-900/30 md:bg-white/95 md:dark:bg-[#1f1f1f]/95 rounded-2xl md:rounded-3xl md:shadow-[20px_20px_50px_-12px_rgba(0,0,0,0.3)] md:border border-slate-200/50 dark:border-slate-800/50 py-2 md:py-3 z-[61] md:backdrop-blur-2xl animate-in fade-in slide-in-from-top-2 md:slide-in-from-left-4 duration-300">
                          <div className="px-5 py-2 mb-1 hidden md:block">
                            <span className="text-[10px] font-black uppercase tracking-widest text-[#077d8a] dark:text-[#58b8c1] opacity-60">Pick Major</span>
                          </div>
                          <div className="max-h-[300px] overflow-y-auto hide-scroll flex flex-col px-1">
                            {availableMajors
                              .filter(m => semesterMajorMappings.some(map => map.semester_id === s.id && map.major_id === m.id))
                              .map(m => (
                                <div
                                  key={m.id}
                                  onClick={(e) => {
                                  ...
                                  className={`mx-2 px-4 py-2.5 rounded-xl text-xs font-bold cursor-pointer transition-all duration-200 ${activeMajorId == m.id && activeSemesterId == s.id ? 'text-white bg-gradient-to-r from-[#045c66] to-[#077d8a] shadow-md shadow-[#077d8a]/20' : 'text-slate-500 dark:text-slate-400 hover:text-[#077d8a] dark:hover:text-[#58b8c1] hover:bg-[#077d8a]/5 dark:hover:bg-[#077d8a]/10'}`}
                                >
                                  {m.name}
                                </div>
                              ))}
              ...
              {isDocument ? (
                ...
                tabs.map((tab) => (
                  <button
                    ...
                    className={`px-5 py-2.5 rounded-full font-bold text-sm transition-all duration-300 whitespace-nowrap snap-start shrink-0 ${activeTab === tab
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <aside className={`absolute md:relative w-[85%] h-full bg-white/90 dark:bg-[#171717]/90 backdrop-blur-xl border-r border-slate-200/60 dark:border-slate-700/60 flex flex-col shadow-2xl md:shadow-none z-30 transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] shrink-0 ${isSidebarOpen
          ...
          <div className="p-6 h-full flex flex-col overflow-y-auto hide-scroll">
            ...
            <div className="mt-auto pt-6">
              ...
              >
                ...
                <span className="text-sm font-bold opacity-70 group-hover:opacity-100">Sayargyi Access</span>
              </Link>
            </div>
          </div>
        </aside>
        ...
        <main className="flex-1 overflow-y-auto w-full bg-slate-50/50 dark:bg-slate-950/60 relative hide-scroll">
          ...
          <div className={`mx-auto p-4 sm:p-6 md:p-10 lg:p-12 relative z-10 ${isDocument ? 'max-w-6xl' : 'max-w-5xl'}`}>

            <header className="mb-6 md:mb-8 flex flex-col md:flex-row justify-between items-start md:items-end gap-4 md:gap-6 shrink-0 border-b border-slate-200/60 dark:border-slate-700/60 pb-6 md:pb-8">
              <div className="flex-1">
                ...
                <p className="text-slate-500 dark:text-slate-300 mt-3 text-base md:text-lg font-medium flex items-center flex-wrap gap-3">
                  ...
                  {activeSubject === "Analysis of Algorithm" && (
                    <button
                      onClick={() => navigate('/practice/analysis-of-algorithm')}
                      className="bg-[#077d8a] hover:bg-[#066d79] text-white px-3 py-1 rounded-lg text-sm font-bold shadow-sm transition-all flex items-center gap-1.5"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  {activeSubject && activeSubject.includes("SQM") && (
                    <button
                      onClick={() => navigate('/practice/sqm')}
                      className="bg-[#077d8a] hover:bg-[#066d79] text-white px-3 py-1 rounded-lg text-sm font-bold shadow-sm transition-all flex items-center gap-1.5"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" />
              ...
              <div
                onClick={toggleSound}
                className={`bg-white/80 dark:bg-[#171717]/80 backdrop-blur-sm border-2 text-rose-600 dark:text-rose-300 px-7 py-4 rounded-3xl flex flex-col items-center justify-center shadow-lg w-full md:w-auto md:min-w-[15rem] cursor-pointer hover:-translate-y-1 active:translate-y-0 active:scale-95 transition-all duration-300 select-none group ${isPlaying ? 'border-rose-400 dark:border-rose-500 bg-rose-50 dark:bg-rose-900/20 shadow-rose-200/60' : 'border-rose-100 dark:border-slate-700 hover:bg-rose-50 dark:hover:bg-slate-800 hover:border-rose-200 dark:hover:border-rose-500/60 shadow-rose-100/50'
                  }`}
              >
            ...
            {isDocument ? (
              <div className="w-full bg-slate-200 dark:bg-[#171717] rounded-3xl shadow-md border border-slate-300 dark:border-slate-700 overflow-hidden flex flex-col h-[calc(100vh-12rem)] md:h-[calc(100vh-16rem)] min-h-[400px] md:min-h-[500px]">
                <object
                  ...
                  {totalQs > 0 && (
                    <div className="flex justify-between items-center mb-4 px-1">
                      <div className="text-sm font-bold flex items-center gap-2">
                        <span className={`px-2 py-1 rounded-md transition-colors ${memorizedCount === totalQs ? 'text-emerald-600 dark:text-emerald-300 bg-emerald-500/10 dark:bg-emerald-500/20' : 'text-slate-400 dark:text-slate-300'
                          }`}>
              ...
              {isDocument ? (
                ...
                <>
                  ...
                  <button
                    ...
                    className={`mt-3 w-full py-2.5 rounded-xl font-bold text-sm transition-all ${canSendOptionalMessage
      ...
      <button
        type="button"
        className={`fixed bottom-6 right-6 z-40 w-16 h-16 rounded-full bg-white dark:bg-[#171717] shadow-2xl border-[3px] border-[#077d8a] hover:scale-105 active:scale-95 transition-all flex items-center justify-center ${(isAiSidebarOpen || isMobileAiModalOpen) ? 'opacity-0 scale-50 pointer-events-none' : 'opacity-100 scale-100'
          }`}
        onClick={() => {
      ...
      {/* Mobile AI Modal */}
      <div
        className={`lg:hidden fixed inset-0 z-[85] transition-all duration-200 ${isMobileAiModalOpen ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'
          }`}
      >
        <div
              ...
              {isDocument ? (
                ...
                <>
                  ...
                  <button
                    ...
                    onClick={sendOptionalMessageToAi}
                    disabled={!canSendOptionalMessage}
                    className={`mt-2 w-full py-2.5 rounded-xl font-bold text-sm transition-all ${canSendOptionalMessage
                      ? 'bg-[#077d8a] text-white hover:bg-[#066d79] active:scale-[0.99]'
                      : 'bg-slate-200 dark:bg-slate-700 text-slate-400 dark:text-slate-300 cursor-not-allowed'
                      }`}
                  >
