...
import practiceData from '../data/fundamental_qa.json';

export default function FundamentalPractice({ onBack }) {
  ...
  const handleReveal = () => {
    ...
    if (currentQ.type === 'fitb') {
      ...
      correct = currentQ.acceptableAnswers
        ? currentQ.acceptableAnswers.some(ans => ans.toLowerCase() === userAns)
        : currentQ.answer.toLowerCase() === userAns;
    } else {
      if (!selectedOption) return;
      
      const cleanOpt = selectedOption.replace(/^[A-Da-d]\.\s*/, '').trim();
      const cleanAnswerText = currentQ.answer.replace(/^[A-Da-d]\.\s*/, '').trim();
      correct = cleanOpt === cleanAnswerText || selectedOption.trim() === currentQ.answer.trim();
    }
  ...
  React.useEffect(() => {
    const handleKeyDown = (e) => {
      if (e.key === 'Enter') {
        if (!showAnswer) {
          if (currentQ.type === 'fitb' ? textAnswer.trim() : selectedOption) handleReveal();
        } else {
          handleNext();
        }
      }
    ...
    <div className="max-w-4xl mx-auto w-full px-4 h-[calc(100vh-120px)] flex flex-col justify-center animate-in fade-in duration-500">
      ...
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg bg-[#6366f1] text-white flex items-center justify-center font-bold shadow-md">F</div>
          <h2 className="text-xl md:text-2xl font-black text-slate-800 dark:text-slate-100 tracking-tight">Fundamentals Practice</h2>
        </div>
      ...
      <div className="bg-white dark:bg-[#171717] rounded-[2rem] shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col max-h-full">
        ...
        {/* Options / Input Area */}
        <div className="p-4 md:p-6 bg-slate-50/30 dark:bg-black/10 overflow-y-auto w-full">
          {currentQ.type === 'fitb' ? (
            <div className="flex flex-col gap-4">
              <input 
                ...
                autoFocus
                className={`w-full p-4 rounded-2xl border-2 bg-white dark:bg-slate-800/50 text-base md:text-lg font-semibold transition-all outline-none focus:ring-2 focus:ring-[#6366f1]/20 ${
                  showAnswer 
                  ? (isCorrect 
                      ? 'border-emerald-500 text-emerald-700 bg-emerald-50 dark:bg-emerald-900/20' 
                      : 'border-rose-500 text-rose-700 bg-rose-50 dark:bg-rose-900/20')
                  : 'border-slate-200 dark:border-slate-700 focus:border-[#6366f1]'
                }`}
              />
            </div>
          ) : (
        ...
        <div className="px-6 py-4 bg-white dark:bg-[#171717] border-t border-slate-100 dark:border-slate-800">
            {!showAnswer ? (
                <button 
                  ...
                  disabled={currentQ.type === 'fitb' ? !textAnswer.trim() : !selectedOption}
                  className={`w-full py-3.5 rounded-2xl font-black text-sm uppercase tracking-widest transition-all shadow-lg active:scale-95 ${
                    (currentQ.type === 'fitb' ? textAnswer.trim() : selectedOption)
