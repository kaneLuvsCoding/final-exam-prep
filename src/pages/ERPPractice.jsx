...
import practiceData from '../data/erp_qa.json';

export default function ERPPractice({ onBack }) {
  ...
  // Safeguard in case data is empty
  const totalQ = practiceData ? practiceData.length : 0;
  ...
  if (!currentQ) return <div className="p-8 text-center text-slate-500">Loading...</div>;
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[80vh] p-6 text-center animate-in fade-in zoom-in duration-500">
        ...
        <p className="text-slate-500 dark:text-slate-400 text-lg mb-8">
          Final Score: <span className={`font-black ${isFail ? 'text-rose-500' : 'text-[#077d8a]'}`}>{score}</span> / {totalQ}
        </p>
    ...
    <div className="max-w-4xl mx-auto w-full px-4 h-[calc(100vh-120px)] flex flex-col justify-center animate-in fade-in duration-500">
      ...
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg bg-[#077d8a] text-white flex items-center justify-center font-bold shadow-md">E</div>
          <h2 className="text-xl md:text-2xl font-black text-slate-800 dark:text-slate-100 tracking-tight">ERP Practice</h2>
        </div>
        <div className="flex items-center gap-4 text-xs font-black uppercase tracking-wider text-slate-400">
          <button
            onClick={() => setIsMuted(!isMuted)}
            className={`hover:text-[#077d8a] transition-colors flex items-center gap-1.5 ${isMuted ? 'text-rose-400 opacity-80' : ''}`}
      ...
      <div className="w-full bg-slate-200 dark:bg-slate-800 rounded-full h-1.5 mb-6 relative overflow-hidden">
        <div
          className="bg-[#077d8a] h-full rounded-full transition-all duration-700 ease-out shadow-[0_0_8px_rgba(249,115,22,0.5)]"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {currentQ.options.map((opt, i) => {
              ...
              if (showAnswer) {
                ...
                const isCorrectOpt = (cleanOptContent === cleanAnswerContent && cleanAnswerContent !== "") || (opt.trim() === (currentQ.answer || "").trim() && currentQ.answer);
                if (isCorrectOpt) {
            ...
            <div className="flex items-center gap-4 animate-in slide-in-from-bottom-2 duration-300 font-bold">
              <div className={`flex-1 p-3 rounded-xl flex items-center gap-3 border ${isCorrect ? 'bg-emerald-50/50 border-emerald-200 text-emerald-800 dark:text-emerald-400' : 'bg-rose-50/50 border-rose-200 text-rose-800 dark:text-rose-400'}`}>
                ...
                <div className="text-xs">
                  ...
                  {!isCorrect && currentQ.answer && <p className="truncate max-w-[150px] md:max-w-none">Ans: {currentQ.answer}</p>}
                  {!isCorrect && !currentQ.answer && <p className="truncate max-w-[150px] md:max-w-none opacity-50">Ans TBD</p>}
                </div>
              </div>
              <button
                onClick={handleNext}
