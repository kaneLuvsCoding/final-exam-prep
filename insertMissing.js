...
import { erp } from "./src/data/erp.js";
...
async function run() {
  ...
  async function processApp(subjectName, dataObj) {
    ...
    for (const [topicName, qList] of Object.entries(dataObj)) {
      ...
      // If topic doesn't exist, we should create it
      if (!topic) {
      ...
      for (const item of qList) {
        if (!existingSet.has(item.question.trim())) {
          ...
          if (error) {
            ...
            inserted++;
            existingSet.add(item.question.trim()); // Prevent duplicates in same run
  ...
  await processApp("BMIS", bmis);
  await processApp("ERP", erp);

  console.log(`Migration complete. Inserted ${inserted} missing questions.`);
}
